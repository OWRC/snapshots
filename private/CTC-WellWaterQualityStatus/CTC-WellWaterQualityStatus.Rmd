...
# Crop to CTC
wells.loc <- wells.loc %>%
  merge(st_join(ctc.bound,
                wells.loc %>% st_as_sf(coords = c("LONG", "LAT"), crs = "NAD83"),
                join=st_contains) %>% select('LOC_ID'),
             by='LOC_ID') %>% 
  st_drop_geometry() %>%
  select(-'geometry')
...
#   dplyr::select(-c(LOC_ID,LOC_NAME,LOC_NAME_ALT1,GRND_ELEV,intersection)) %>%
#   st_drop_geometry() 

wells.loc.rfmt <- wells.loc %>% 
  mutate(Municipality = Upp_Muni) %>%
  dplyr::select(-c(LOC_ID,LOC_NAME,LOC_NAME_ALT1,GRND_ELEV)) 
...
### Wells outside the CTC

In addition to the CTC Source Protection Region wells, municipal well data added to the ORMGP database are also included in the analysis. Many of the wells are not accompanied with water quality information. Should municipalities wish to include their water quality data within this snapshot, please [contact the ORMGP](mailto:support@owrc.ca).
...
## Generalized Additive Mixed Model (GAMM) {#gamm}
...
<!-- > Note: The future projection concentrations for some wells predict a concentration of 0 mg/L and a negative magnitude of change. For the sake of transparency, these projection values will remain as currently presented, as these results are based on a statistical model projection and are thus not definitive. Please use your better judgement. -->

<br>

### Chloride by well

Summary of chloride status, trend, and projections for municipal production wells in the CTC SPR. For the current period, concentrations are based on median observed values, and trends are based on GAMM results. For 2040 projections, concentrations and trend directions are based on GAMMs, and likelihood of exceedance is based on comparison of GAMM and linear regression.


```{r tbl.Cl, echo=FALSE, warning=FALSE}
wells.loc.rfmt %>%
  drop_na() %>%
  mutate(Location_Municipality=str_to_title(Municipality),Location_Well=Well) %>%
  ...
  mutate('Current_Comparison to ODWQS'=if_else(Cl.current<250,'Below','Above')) %>%
...
Summary of sodium status, trend, and projections for municipal production wells in the CTC SPR. For the current period, concentrations are based on median observed values, and trends are based on GAMM results. For 2040 projections, concentrations and trend directions are based on GAMMs, and likelihood of exceedance is based on comparison of GAMM and linear regression.
...
Summary of nitrate status, trend, and projections for municipal production wells in the CTC SPR. For the current period, concentrations are based on median observed values, and trends are based on GAMM results. For 2040 projections, concentrations and trend directions are based on GAMMs, and likelihood of exceedance is based on comparison of GAMM and linear regression.
...
Status of each well in reference to the applicable half-Maximum Allowable Concentration and ODWQS objectives based on the most recent year of data. Please refer to [the description above](#gamm) for more information on the exceedance likelihoods and confidence.

`Hover over any point to reveal municipal well properties; click point for more details; click on image to enlarge` 

```{r mappal, echo=FALSE, warning=FALSE}
  pal <- colorNumeric(c('#4daf4a','#984ea3','#e41a1c'), 0:2)
```
...
leaflet(ctc.bound) %>%
  ...
  addCircleMarkers(data = wells.loc %>% filter(is.na(Cl.status) & is.na(Cl.current)),
                   lat = ~LAT, lng= ~LONG,
                   radius = 2,
                   color = 'black', #'#999999',
                   stroke = FALSE,
                   opacity = 1,
                   fillOpacity = 1,
                   group="Show all municipal wells",
                   label = ~Well,
  ...
  addCircleMarkers(data = wells.loc %>% filter(is.na(Cl.status) & !is.na(Cl.current)),
                   ...
                   popup = ~paste0(#'<style> div.leaflet-popup-content {width:auto !important;}</style>',
                                   ...
                                   '<br><a href="https://golang.oakridgeswater.ca/pages/CTC/Chloride/', LOC_ID, '.png" target="_blank"><img src = https://golang.oakridgeswater.ca/pages/CTC/Chloride/', LOC_ID, '.png width="600"></a>',
  ...
  addLegendCustom(colors = c('#ff7f00','#4daf4a','#984ea3','#e41a1c','white'), 
                  labels = c('limited data','unlikely','somewhat likely','highly likely','CTC jurisdiction'), 
                  sizes = c(10,10,15,20,20), 
                  shapes = c('circle','circle','circle','circle','square'), 
                  borders = c('#ff7f00','#4daf4a','#984ea3','#e41a1c','black'), 
                  opacity = 1.) %>%
  addLayersControl(
      overlayGroups = "Show all municipal wells",
      position = 'bottomright',
      options = layersControlOptions(collapsed = FALSE)
    ) %>% hideGroup("Show all municipal wells")
```
...
leaflet(ctc.bound) %>%
  ...
  addCircleMarkers(data = wells.loc %>% filter(is.na(Na.status) & is.na(Na.current)),
                   lat = ~LAT, lng= ~LONG,
                   radius = 2,
                   color = 'black', #'#999999',
                   stroke = FALSE,
  ...
  addCircleMarkers(data = wells.loc %>% filter(is.na(Na.status) & !is.na(Na.current)),
                   ...
                   popup = ~paste0(#'<style> div.leaflet-popup-content {width:auto !important;}</style>',
                                   ...
                                   '<br><a href="https://golang.oakridgeswater.ca/pages/CTC/Sodium/', LOC_ID, '.png" target="_blank"><img src = https://golang.oakridgeswater.ca/pages/CTC/Sodium/', LOC_ID, '.png width="600"></a>',
                                   '<br>click image to expand'),
                   options = popupOptions(maxWidth = 1000)                 
                   ) %>%
...
leaflet(ctc.bound) %>%
  ...
  addCircleMarkers(data = wells.loc %>% filter(is.na(NO3.status) & is.na(NO3.current)),
                   lat = ~LAT, lng= ~LONG,
                   radius = 2,
                   color = 'black', #'#999999',
                   stroke = FALSE,
  ...
  addCircleMarkers(data = wells.loc %>% filter(is.na(NO3.status) & !is.na(NO3.current)),
                   ...
                   popup = ~paste0(#'<style> div.leaflet-popup-content {width:auto !important;}</style>',
                                   ...
                                   '<br><a href="https://golang.oakridgeswater.ca/pages/CTC/Nitrate/', LOC_ID, '.png" target="_blank"><img src = https://golang.oakridgeswater.ca/pages/CTC/Nitrate/', LOC_ID, '.png width="600"></a>',
                                   '<br>click image to expand'),
                   options = popupOptions(maxWidth = 1000)                 
                   ) %>%
...
Summary of municipal production well trends. 

Notes:

1. For the *current* period, concentrations are based on median observed values, and trends are based on GAMM results. 
1. For *2040 projections*, concentrations and trend directions are based solely on GAMMs, 
1. Trends are deemed when GAMM slopes exceed $\pm$ `r trnd.trsh` mg/L/year
1. likelihood of exceedance is based on [comparison of GAMM and linear regression](#gamm). 
...
leaflet(ctc.bound) %>%
  ...
  addMarkers(data = wells.loc %>% filter(!is.na(Cl.status), Cl.fut.trnd > trnd.trsh),
             lat = ~LAT, lng= ~LONG,
             icon = icons.up,
             label = ~Well, 
             group="Future trend",
  ...
  addLayersControl(
      baseGroups = c('Current trend','Future trend'),
      overlayGroups = "Show all municipal wells",
      position = 'bottomright',
      options = layersControlOptions(collapsed = FALSE)
    ) %>% hideGroup("Show all municipal wells") 
...
leaflet(ctc.bound) %>%
  ...
  addMarkers(data = wells.loc %>% filter(!is.na(Na.status), Na.fut.trnd > trnd.trsh),
             lat = ~LAT, lng= ~LONG,
             icon = icons.up,
             label = ~Well, 
             group="Future trend",
...
leaflet(ctc.bound) %>%
  ...
  addMarkers(data = wells.loc %>% filter(!is.na(NO3.status), NO3.fut.trnd > trnd.trsh),
             lat = ~LAT, lng= ~LONG,
             icon = icons.up,
             label = ~Well, 
             group="Future trend",
