...
RunCVCscript <- function(csvfp, dfloc, endyear=2040) {
    ...
    map_dfr(
      function(Well.Names) {
        ...
        dat = bind_rows(Wells.Wide %>%
                          ...
                          distinct() %>%
                          mutate(Presence = 1) %>%
                          full_join(tibble(Year = 1950:2023)) %>%
                          arrange(Year) %>%
                          filter(Year %in% c(Start.Cl:2023)) %>%
                        ...
                        Wells.Wide %>%
                          ...
                          full_join(tibble(Year = 1950:2023)) %>%
                          arrange(Year) %>%
                          filter(Year %in% c(Start.Na:2023)) %>%
                          mutate(Presence = ifelse(is.na(Presence), "_", Year),
                                 Well = Well.Names,
                                 Parameter = "Sodium") %>%
                          pivot_wider(names_from = "Year", values_from = "Presence") %>%
                        ...
                        Wells.Wide %>%
                          ...
                          full_join(tibble(Year = 1950:2023)) %>%
                          arrange(Year) %>%
                          filter(Year %in% c(Start.Ni:2023)) %>%
                          mutate(Presence = ifelse(is.na(Presence), "_", Year),
                                 Well = Well.Names,
                                 Parameter = "Nitrate") %>%
                          pivot_wider(names_from = "Year", values_from = "Presence") %>%
  ...
  Wells.Seasonal <- inner_join(Wells %>%
                                 filter(!is.na(Value)) %>%
  ...
  neat_K5_K3 = function(muni, well_id, para) {
    ...
    Year_Range = 2022 - First_Point
    Xmin = ifelse(Year_Range < 10, Start, floor(First_Point/5)*5)
    By = case_when((2022 - Xmin) %in% 20:29 ~ 5,
                   (2022 - Xmin) > 29 ~ 10,
                   (2022 - Xmin) < 20 ~ 2)
    ...
    ggplot(Wells %>% filter(Muni == muni & Well == well_id & Parameter == para), aes(Time, Value)) +
      ...
      scale_x_continuous(breaks = seq(Xmin, 2022, by = By), limits = c(Xmin, 2022), name = NULL) +
      scale_y_continuous(name = paste0(para, " (mg/L)"), limits = c(Ymin, Max), expand = c(0, 0)) +
      theme_bw() +
      ggtitle(paste(Well_Name,"(Annual)", sep = " ")) + 
  ...
  neat_S = function(muni, well_id, para) {
    ...
    GAM.S.Model <- gamm(Value ~ s(Year, bs = "cr", k = K1) + s(Month, bs = "cc", k = K2) + ti(Year, Month, bs = c("cr","cc"), k = c(K1, K2)),
                        data = Wells.S %>% filter(Muni == muni & Well == well_id & Parameter == para & Year < 2023), method = "ML",
    ...
    ggplot(pdat.New.S, aes(x = Month, y = fitted, group = fYear)) +
      ...
      scale_y_continuous(name = paste0(para, " (mg/L)"), limits = c(Ymin, Max), expand = c(0, 0)) +
      theme_bw() +
      ggtitle(paste(Well_Name, "(Monthly)", sep = " ")) + 
  ...
  neat_K5_LM.P = function(muni, well_id, para) {
    ...
    By = case_when((2022 - Xmin) > 29 ~ 15,
                   (2022 - Xmin) < 29 ~ 10)
    
    # Specify k
    K = Wells.K %>% filter(Muni == muni & Well == well_id & Parameter == para) %>% pull(K)
    Bucket5 = round((2022 - Start)/(K - 1), 0)
    ...
    ggplot(Wells %>% filter(Muni == muni & Well == well_id & Parameter == para), aes(Year, Value)) +
      ...
      ggtitle(paste(Well_Name,"(Projections)")) + 
      theme(plot.title = element_text(hjust = 0.5, size = 13),
            axis.text.y = element_text(size = 12),
            axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1, size = 12),
            axis.title = element_text(size = 13),
  ...
  Seasonal.List %>%
    pmap(function(Muni, Well, Parameter){
      ...
      #          path = 'dat/gammfigs',
      #          file = paste0(dfloc$LOC_ID[dfloc$Name==Well],"-",Parameter, ".png"),
      #          width = 24.5,
      #          height = 9.5,
      #          dpi = 300,
      #          units = "cm")
      ...
      Assembly.Two(muni = Muni, well_id = Well, para = Parameter) %>%
        ggsave(# path = "dat/non-seasonal",
          ...
          file = paste0(dfloc$LOC_ID[dfloc$Name==Well], ".png"),
          width = 17,
          height = 9.5,
          dpi = 300,
          units = "cm")
